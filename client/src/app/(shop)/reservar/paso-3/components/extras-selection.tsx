...
export function ExtrasSelection() {
  ...
  const ExtraCard = ({ extra }: { extra: Extra }) => {
    ...
    return (
      <motion.div
        layout // Agregamos layout para animaciones suaves de cambios de estado
        initial={{ opacity: 0, y: 20 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ duration: 0.3 }}
        className="h-full" // Aseguramos altura consistente
      >
        <Card
    ...
    <div className="space-y-6">
      ...
      <section className="space-y-4">
        ...
        <div className="grid gap-4 md:grid-cols-2">
          {protectionExtras.map((extra) => (
            <motion.div key={extra.id} layout="position">
      ...
      <Card className="mt-8">
        ...
        <div className="p-6 pt-0 flex flex-col sm:flex-row sm:justify-end gap-4">
          <Button
            variant="outline"
            onClick={() => router.push("/reservar/paso-2")}
          >
            Atrás
          </Button>
          <Button onClick={() => router.push("/reservar/paso-4")}>
            Continuar
          </Button>
        </div>
      </Card>
